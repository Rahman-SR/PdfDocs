...
const plans = [
  { name: 'Free', subtitle: 'Perfect for quick, one-off tasks.', price: '$0', action: 'Get Started', features: ['Basic tools (Merge, Split)', '5 tasks per day', 'Max 50MB per file'] },
  { name: 'Pro', subtitle: 'For power users and professionals.', price: '$12', action: 'Get Started', popular: true, features: ['Unlimited tools & tasks', 'Batch processing', 'Up to 2GB file size', 'OCR Text Recognition'] },
  ...
  return (
    <main>
      <section className="px-5 pb-16 pt-14 text-center sm:px-8 sm:pt-18">
        ...
        <div className="mt-7 inline-flex items-center gap-3 text-xs"><span>Monthly</span><span className="relative h-6 w-11 rounded-full bg-primary"><span className="absolute right-1 top-1 size-4 rounded-full bg-white" /></span><span className="text-muted">Yearly <span className="text-emerald-700">Save 20%</span></span></div>
      ...
      <section className="px-5 py-16 sm:px-8"><div className="mx-auto max-w-6xl rounded-2xl bg-[#2774e8] px-6 py-12 text-center text-white"><h2 className="font-display text-3xl font-semibold sm:text-4xl">Ready to optimize your workflow?</h2><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-white/75">Join professionals who trust PDF Toolkit for daily document needs.</p><div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row"><Link to="/login" className="inline-flex min-h-11 items-center justify-center rounded-lg bg-white px-5 text-sm font-medium text-primary">Get Started for Free</Link><a href="mailto:sales@example.com" className="inline-flex min-h-11 items-center justify-center rounded-lg border border-white/35 px-5 text-sm font-medium">Talk to Sales</a></div></div></section>
    </main>
  )
}

function PriceCard({ name, subtitle, price, action, features, popular = false }: { name: string; subtitle: string; price: string; action: string; features: string[]; popular?: boolean }) {
  return <article className={`relative flex min-h-96 flex-col rounded-xl border bg-white p-7 soft-shadow ${popular ? 'border-primary ring-1 ring-primary/10' : 'border-line'}`}>{popular && <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-primary px-3 py-1 text-[9px] font-semibold uppercase tracking-wider text-white">Most Popular</span>}<h2 className="font-display text-lg font-medium">{name}</h2><p className="mt-2 text-xs text-muted">{subtitle}</p><p className="mt-7 font-display text-4xl font-semibold tracking-[-0.035em]">{price}{price.startsWith('$') && <span className="ml-1 text-xs font-normal text-muted">/mo</span>}</p><ul className="mt-7 flex-1 space-y-3 text-xs text-muted">{features.map((item) => <li key={item} className="flex items-center gap-2"><Check className="size-3.5 text-primary" />{item}</li>)}</ul><Link to="/login" className={`mt-7 flex min-h-10 items-center justify-center rounded-lg border text-xs font-medium ${popular ? 'border-primary bg-primary text-white' : name === 'Business' ? 'border-[#292d2e] bg-[#292d2e] text-white' : 'border-[#b8bec8]'}`}>{action}</Link></article>
}
