  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { data, error: signUpError } = await supabase.auth.signUp({
        ...
        password,
        options: { emailRedirectTo: buildAuthRedirect('/auth/callback', returnTo) },
      })
    ...
    <main className="grid min-h-screen bg-canvas lg:grid-cols-[1.18fr_0.82fr]">
      <section className="flex min-h-screen items-center justify-center px-5 py-12 sm:px-8">
        <div className="w-full max-w-md">
          <Link to="/" className="mx-auto flex w-fit flex-col items-center text-center"><span className="grid size-14 place-items-center rounded-2xl bg-gradient-to-br from-cyan-500 via-blue-600 to-violet-700 text-white shadow-[0_0_28px_rgba(37,99,235,0.35)]"><FileText className="size-7" /></span><span className="mt-4 font-display text-2xl font-bold">PdfDocs</span><span className="mt-1 text-sm text-muted">Private PDF workspace</span></Link>
          <div className="mt-9 rounded-xl border border-line bg-white p-6 soft-shadow sm:p-8">
            ...
            <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
              <AuthField label="Email address" icon={Mail}><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="email" maxLength={254} required placeholder="name@company.com" className="auth-input" /></AuthField>
              <AuthField label="Password" icon={LockKeyhole} action={mode === 'signin' ? <Link to="/forgot-password" className="text-xs font-medium text-primary">Forgot password?</Link> : undefined}><input type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete={mode === 'signin' ? 'current-password' : 'new-password'} minLength={mode === 'signup' ? MIN_PASSWORD_LENGTH : undefined} maxLength={MAX_PASSWORD_LENGTH} required placeholder={mode === 'signup' ? 'Create a strong password' : 'Enter your password'} className="auth-input pr-12" /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute bottom-1 right-1 grid size-10 place-items-center rounded-lg text-muted hover:bg-[#f3f4f5]" aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? <EyeOff className="size-4" /> : <Eye className="size-4" />}</button></AuthField>
              {mode === 'signup' && <><AuthField label="Confirm password" icon={LockKeyhole}><input type={showPassword ? 'text' : 'password'} value={passwordConfirmation} onChange={(event) => setPasswordConfirmation(event.target.value)} autoComplete="new-password" minLength={MIN_PASSWORD_LENGTH} maxLength={MAX_PASSWORD_LENGTH} required placeholder="Repeat your password" className="auth-input" /></AuthField><p className="text-xs leading-5 text-muted">{PASSWORD_REQUIREMENTS}</p></>}
            ...
            <p className="mt-5 text-center text-xs leading-5 text-muted">Email and password are the only supported sign-in method.</p>
          </div>
